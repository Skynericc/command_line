...
module.exports = function (app, passport) {
  ...
  app.get("/ls", function (req, res) {
    var path =
      "./user_data" + "/" + req.user.local.email + "/" + req.query.directory;
  ...
  app.get("/autoprediction", function (req, res) {
    ...
    if (foldername[0] == "/") {
      ...
      foldername = foldername.substr(
        11 + req.user.local.email.length + req.query.directory.length,
        temp.length
      );
      if (foldername.includes("/") == true) {
        ...
        dir =
          req.query.directory +
          foldername.slice(0, foldername.lastIndexOf("/"));
        dir += "/";
      } else {
        path = user_path;
      ...
      var last_dir_of_foldername = foldername.slice(
      ...
      response = predictionFolder.ListFiles(
        foldername,
        path,
        last_dir_of_foldername
      );
  ...
  app.get("/cd", function (req, res) {
    ...
    if (foldername[0] == "/") {
      ...
      path_folder =
        "./" + path_folder.substr(__dirname.length - 5, path_folder.length);
      ...
      foldername = path_folder.substr(
        default_path.length + 1,
        path_folder.length
      );
  ...
  app.get("/delete", function (req, res) {
    if (req.isAuthenticated()) {
      if (!req.query.nameFolder.match(format)) {
        var path_folder = req.query.directory + "/" + req.query.nameFolder;
        var response = DirFunctions.deleteDir(
          path_folder,
          req.user.local.email
        );
        if (response.constructor === Error) {
